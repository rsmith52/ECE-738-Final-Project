...
train_imgs_dir = os.path.join(cityscapes_dir, "leftImg8bit", "train")
train_gt_dir = os.path.join(cityscapes_dir, "gtFine", "train")

val_imgs_dir = os.path.join(cityscapes_dir, "leftImg8bit", "val")
val_gt_dir = os.path.join(cityscapes_dir, "gtFine", "val")
...
for dir_step, d in enumerate(train_dirs):
    img_dir = os.path.join(train_imgs_dir, d)
    ...
    for step, file_name in enumerate(file_names):
        ...
        gt_img_path = os.path.join(train_gt_dir, d, img_id + "_gtFine_labelIds.png")
...
# # save to disk:
cPickle.dump(mean_channels, open(os.path.join(project_dir, "data", "mean_channels.pkl"), "wb"))
...
# # save to disk:
cPickle.dump(class_weights, open(os.path.join(project_dir, "data", "class_weights.pkl"), "wb"))


# get the path to all validation images and their corresponding label image:
val_img_paths = []
val_trainId_label_paths = []
for dir_step, d in enumerate(val_dirs):
    img_dir = os.path.join(val_imgs_dir, d)
    ...
    for step, file_name in enumerate(file_names):
        ...
        img_small = cv2.resize(img, (new_img_width, new_img_height),
                    interpolation=cv2.INTER_NEAREST)
        img_small_path = os.path.join(project_dir, "data", img_id + ".png")
        cv2.imwrite(img_small_path, img_small)
        ...
        # (want the resulting image to still only contain pixel values
        # corresponding to an object class):
        gt_img_path = os.path.join(val_gt_dir, d, img_id + "_gtFine_labelIds.png")
        gt_img = cv2.imread(gt_img_path, -1)
        gt_img_small = cv2.resize(gt_img, (new_img_width, new_img_height),
                    interpolation=cv2.INTER_NEAREST)
        ...
        # save the label image to project_dir/data:
        trainId_label_path = os.path.join(project_dir, "data", img_id + "_trainId_label.png")
        cv2.imwrite(trainId_label_path, trainId_label)
        val_trainId_label_paths.append(trainId_label_path)

# # save the validation data to disk:
cPickle.dump(val_trainId_label_paths,
            open(os.path.join(project_dir, "data", "val_trainId_label_paths.pkl"), "wb"))
cPickle.dump(val_img_paths,
            open(os.path.join(project_dir, "data", "val_img_paths.pkl"), "wb"))
...
for step, (img_path, label_path) in enumerate(zip(train_img_paths, train_trainId_label_paths)):
    ...
    label_img_flipped_path = os.path.join(label_path.split(".png")[0], "_flipped.png")
...
cPickle.dump(train_img_paths,
            open(os.path.join(project_dir, "data", "train_img_paths.pkl"), "wb"))
cPickle.dump(train_trainId_label_paths,
            open(os.path.join(project_dir, "data", "train_trainId_label_paths.pkl"), "wb"))
