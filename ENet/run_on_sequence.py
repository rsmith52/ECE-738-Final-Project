...
seq_frames_dir = os.path.join("data", "cityscapes", "leftImg8bit", "demoVideo", "stuttgart_02")
seq_frame_paths = []
...
for step, frame_name in enumerate(frame_names):
    ...
    frame_path = os.path.join(seq_frames_dir, frame_name)
...
results_dir = os.path.join(model.project_dir, "results_on_seq")
...
with tf.Session() as sess:
    ...
    saver.restore(sess, os.path.join(project_dir, "training_logs", "best_model", "model_1_epoch_23.ckpt"))
    ...
    for step in range(no_of_batches):
        ...
        for i in range(batch_size):
            ...
            pred_path = os.path.join(results_dir, img_name + "_pred.png")
...
out = cv2.VideoWriter(os.path.join(results_dir, "cityscapes_stuttgart_02_pred.avi"), fourcc,
...
for step, frame_name in enumerate(frame_names):
    ...
    if ".png" in frame_name:
        frame_path = os.path.join(results_dir, frame_name)
