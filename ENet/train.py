...
train_mean_channels = cPickle.load(open(os.path.join("data", "mean_channels.pkl"), 'rb'))

# load the training data from disk:
train_img_paths = cPickle.load(open(os.path.join(data_dir, "train_img_paths.pkl"), 'rb'))
train_trainId_label_paths = cPickle.load(open(os.path.join(data_dir, "train_trainId_label_paths.pkl"), 'rb'))
...
val_img_paths = cPickle.load(open(os.path.join(data_dir, "val_img_paths.pkl"), 'rb'))
val_trainId_label_paths = cPickle.load(open(os.path.join(data_dir, "val_trainId_label_paths.pkl"), 'rb'))
    ...
    for step in range(no_of_val_batches):
        ...
        if step < 4:
            ...
            for i in range(batch_size):
                ...
                cv2.imwrite((os.path.join(model.debug_imgs_dir, "val_" + str(epoch) + "_" +
...
with tf.Session() as sess:
    ...
    for epoch in range(no_of_epochs):
        ...
        cPickle.dump(train_loss_per_epoch, open(os.path.join(model.model_dir, "train_loss_per_epoch.pkl"), "wb"))
        ...
        cPickle.dump(val_loss_per_epoch, open(os.path.join(model.model_dir, "val_loss_per_epoch.pkl"), "wb"))
        print("validation loss: %g" % val_loss)

        if val_loss < max(best_epoch_losses): # (if top 5 performance on val:)
            # save the model weights to disk:
            checkpoint_path = (os.path.join(model.checkpoints_dir, "model_" +
        ...
        plt.savefig(os.path.join(model.model_dir, "train_loss_per_epoch.png"))
        plt.close(1)
        ...
        plt.savefig(os.path.join(model.model_dir, "val_loss_per_epoch.png"))
        plt.close(1)
